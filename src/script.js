  ...
  render(){
    return(
      <div id="wrapper">
        ...
        <div id="numberkeys">
         {numberkeys.map(key=>(
           <button onClick={key=="AC"?this.clear:key ==="."?this.decimal:this.numberHandler}className="key"id={numberids[numberkeys.indexOf(key)]}>{key}</button>

          ))}
        </div>
          <div id="calckeys">
         {calckeys.map(key=>(
           <button className="key"id={calcids[calckeys.indexOf(key)]} 
